...
// 변수 oneDate 라는 딕셔너리에서 String 형태의 key로 String 타입의 값을 저장하기 위한 빈 딕셔너리이며, 회고 시스템을 구현할때 추가, 수정, 삭제한 값들을 모두 저장하기 위해서는 while문 밖에 선언해야함. -> 잘 몰랐었는데 어떻게 알게됐는지 설명하고 처음에는 while 문 안에 해서 계속 초기화됐었음을 설명하기
